...
const draw = _deltaTime => {
  ...
  programInfo.shader.with(() => {
    ...
    programInfo.meshes.forEach(me => {
      ...
      programInfo.shader.uniformVec3("uDiffuseColor", diffuse)
...
const main = () => {
  ...
  programInfo = {
    shader: new Shader({ vertex: vsSource, fragment: fsSource }),
    gpuMeshes: [],
    size: 0,
  }
  ...
  Promise.all([
    ...
    programInfo.meshes = meshes.map(rmesh => {
      ...
      mesh.material = new Material(Vector3.fromArray(rmesh.material.color))
      return mesh
    })
